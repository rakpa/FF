...
export default function Dashboard() {
  ...
  const monthlyData = Array.from({ length: 12 }, (_, month) => {
    ...
    const monthSalary = salaries?.filter(s => s.month === monthNumber).reduce((sum, s) => sum + s.amount, 0) || 0;
    const monthExpenses = expenses?.filter(e => e.month === monthNumber).reduce((sum, e) => sum + e.amount, 0) || 0;
